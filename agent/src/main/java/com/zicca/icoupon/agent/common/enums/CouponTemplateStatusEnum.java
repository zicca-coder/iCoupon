package com.zicca.icoupon.agent.common.enums;
...
public enum CouponTemplateStatusEnum implements IEnum<Integer> {
    ...
    public static CouponTemplateStatusEnum fromDesc(String desc) {
        return Arrays.stream(values())
                .filter(item -> item.desc.equals(desc))
                .findFirst().orElseThrow(() -> new ServiceException("无效的优惠券模板状态描述"));
    }
}
