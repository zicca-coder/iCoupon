package com.zicca.icoupon.coupon.mq.producer;

import cn.hutool.core.util.StrUtil;
import com.zicca.icoupon.coupon.mq.base.BaseExtendDTO;
import com.zicca.icoupon.coupon.mq.base.MessageWrapper;
import com.zicca.icoupon.coupon.mq.event.UserCouponExpireEvent;
import org.apache.rocketmq.common.message.MessageConst;
import org.apache.rocketmq.spring.core.RocketMQTemplate;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.messaging.Message;
import org.springframework.messaging.support.MessageBuilder;
import org.springframework.stereotype.Component;

import java.util.UUID;

import static com.zicca.icoupon.coupon.common.constants.MQConstants.USER_COUPON_EXPIRE_TOPIC;

/**
 * 用户优惠券过期消息生产者
 *
 * @author zicca
 */
@Component
public class UserCouponExpireProducer extends AbstractCommonProducerTemplate<UserCouponExpireEvent> {

    public UserCouponExpireProducer(@Autowired RocketMQTemplate rocketMQTemplate) {
        super(rocketMQTemplate);
    }


    @Override
    protected BaseExtendDTO buildBaseExtendDTO(UserCouponExpireEvent event) {
        return BaseExtendDTO.builder()
                .eventName("用户优惠券过期自动修改状态事件")
                .keys(event.getUserId() + ":" + event.getUserCouponId())
                .topic(USER_COUPON_EXPIRE_TOPIC)
                .sentTimeout(5000L)
                .delayTime(event.getDelayTime())
                .build();

    }

    @Override
    protected Message<?> buildMessage(UserCouponExpireEvent event, BaseExtendDTO baseExtendDTO) {
        String keys = StrUtil.isEmpty(baseExtendDTO.getKeys()) ? UUID.randomUUID().toString() : baseExtendDTO.getKeys();
        return MessageBuilder
                .withPayload(new MessageWrapper(keys, event))
                .setHeader(MessageConst.PROPERTY_KEYS, keys)
                .setHeader(MessageConst.PROPERTY_TAGS, baseExtendDTO.getTag())
                .build();
    }
}
